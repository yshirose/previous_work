##200312 edit optim to compute MRRs

import numpy as np
from numpy.random import shuffle
from numpy import dot
from collections import defaultdict
from skge_are.param import Parameter, AdaGrad
import timeit
import pickle

from skge_are.util import memoized
...
class StochasticTrainer(object):
    ...
    def _optim(self, xys):
        ...
        batch_idx = np.arange(self.batch_size, len(xys), self.batch_size,dtype=int) ##dtype=int
        #print(idx,batch_idx,type(idx[1]),type(batch_idx[1]))## delete
        
        ##
        path="/home/cl/yushi-hi/program/scikit-kge-master/exp/WN18RR/validxs.pkl"
        with open(path, 'rb') as f:
            validxs=pickle.load(f)
        path="/home/cl/yushi-hi/program/scikit-kge-master/exp/WN18RR/allxs.pkl"
        with open(path, 'rb') as f:
            allxs=pickle.load(f)
        path="/home/cl/yushi-hi/program/scikit-kge-master/exp/WN18RR/specific_use/val_alldic.pkl"
        with open(path, 'rb') as f:
            val_alldic=pickle.load(f)
        path='/home/cl/yushi-hi/program/scikit-kge-master/exp/WN18RR/path_features/validpfs.npz'
        validpfs=load_npz(path)
#         self.model.add_hyperparam('filMRRs',[])
#         self.model.add_hyperparam('frawMRRs',[])
        ...
        for self.epoch in range(1, self.max_epochs + 1):
            ...
            if self.epoch%100==0:
                ...
                ss,os,ps=list(zip(*validxs))
                ss=list(ss)
                os=list(os)
                ps=list(ps)
                
                l=[dot(dot(self.model.E[ss],self.model.W[i]),self.model.E.T) for i in range(pn)]
                
                scores=[l[validxs[i][2]][i,validxs[i][1]] for i in range(len(validxs))]
                for i in range(len(val_alldic)):
                    for j in val_alldic[i]:
                        l[j[2]][i,j[1]] = -np.inf
...
#                  #raw mrr
                ...
                for i in range(len(validxs)):
                    csrpf=validpfs[pn*i:pn*(i+1),:]
                    s,o,p=validxs[i]
                    pscore=self.model.wp[p]*csrpf #this case * is matrix multiplication
                    
                    row=l[p][i]+pscore#row=l[validxs[i][2]][i]+pscore
                    val=scores[i]+pscore[o]
                    row[o]=val#row[validxs[i][1]]=val
                    sort=np.sort(row)[::-1]
                    rr=1/(np.where(sort==val)[0][0]+1)
                    rrs.append(rr)


                self.model.filMRRs.append(sum(rrs)/len(rrs))               
            ...
            for batch in np.split(idx, batch_idx):
                ...
                self._process_batch(bxys,batch) #batch is　data idx array of one batch 
    ...
    def _process_batch(self, xys,batch):
        ...
        # take step for batch
        grads = self.model._gradients(xys,batch)
